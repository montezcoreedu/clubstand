<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO account_groups (GroupName) VALUES (?)");
            $stmt->bind_param("s", $groupName);
            if (!$stmt->execute()) {
                throw new Exception("Failed to create account group: " . $stmt->error);
            }
            ...
            if (!empty($permissions)) {
                ...
                if (!$stmt->execute()) {
                    throw new Exception("Failed to assign permissions: " . $stmt->error);
                }
                $stmt->close();
            }

            $conn->commit();
            $_SESSION['successMessage'] = "<div class='message success'>Account group successfully created!</div>";
...
<body>
    ...
    <div id="wrapper">
        ...
        <h2>Add Security Group</h2>
        <form method="post">
            <table class="form-table">
                <tbody>
                    <tr>
                        <td width="180"><b>Group Name:</b></td>
                        <td><input type="text" name="GroupName" maxlength="100" required style="width: 30%;"></td>
                    </tr>
                    <tr>
